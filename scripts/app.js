...
async function GetFiveDayData() {
    ...
    //#region 
        ...
        for (let i = 0; i < n; i++) {
            ...
            earlyTemps[i] = (regressionLine.slope * earlierTime[i] + regressionLine.intercept).toFixed(2);
        }
        ...
        for (let i = 1; i < m; i++) {
            ...
            let regressionLine = linearRegression(x, y);
            laterMaxTemps[i - 1] = (regressionLine.slope * laterTime[i - 1] + regressionLine.intercept).toFixed(2);
        }
        ...
        for (let i = 1; i < m; i++) {
            ...
            let regressionLine = linearRegression(x, y);
            laterMinTemps[i - 1] = (regressionLine.slope * laterTime[i - 1] + regressionLine.intercept).toFixed(2);
        }
